		...
		u16 batch_mode, int sensor_type)
{
	...
	if (data->IsVDIS_Enabled == true && sensor_type == GYROSCOPE_SENSOR) {
		...
		memcpy(&ts_cnt, pchRcvDataFrame + *iDataIdx + 6, 4);
...
normal_parse:
	...
	if (ts_flag == VDIS_TIMESTAMP_FORMAT || ts_flag == SUPER_VDIS_FORMAT) {
		ssp_debug_time("[SSP_DEBUG_TIME] ts_index: %u stacked_cnt: %u ts_flag: %x ts_cnt: %x timestamp: %llu ts_buffer: %llu",
				ts_index, data->ts_stacked_cnt, ts_flag, ts_cnt, time_delta_ns, data->ts_index_buffer[ts_index]);
	} else { 
